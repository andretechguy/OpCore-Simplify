...
download_py () {
    ...
    url="$(curl -L https://www.python.org/downloads/release/python-${vers//./}/ --compressed 2>&1 | grep -iE "python-$vers.tgz\"" | awk -F'"' '{ print $2 }')"
    if [ -z "$url" ]; then
    ...
    echo
    tar -xzf "$tempdir/python.tgz" -C "$tempdir"
    if [ "$?" != "0" ]; then
        echo
